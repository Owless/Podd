...
const ItemCard = ({ item, onDelete, expandedItemId, setExpandedItemId }) => {
  ...
  const handleDeleteClick = async (e) => {
    ...
    setShowModal(true); // Показываем модал
  };
    ...
    <>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-lg max-w-xs w-full mx-auto p-5 transform transition-all scale-100 opacity-100">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Подтверждение удаления</h3>
            <p className="text-sm text-gray-600 mb-4">
              Вы действительно хотите удалить товар <strong>{item.title}</strong> из магазина{' '}
              <strong>{getDomainName(item.url)}</strong>?
            </p>
